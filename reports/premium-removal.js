...
function uniqDates(list){const seen={};return list.filter(Boolean).sort((a,b)=>new Date(b)-new Date(a)).filter(d=>{const k=new Date(d).toISOString();if(seen[k])return false;seen[k]=true;return true})}
...
function analyze(records,v){
 ...
 if (a.is24v) {
   // Mais rigoroso para 24V: penaliza fortemente logo no primeiro reboot
   p += a.reboots === 0 ? 20 : a.reboots === 1 ? 10 : 0;
 } else {
 ...
 if (a.is24v) {
   // Mais rigoroso para 24V: perde mais pontos com menos ocorrências
   p += vb === 0 ? 20 : vb <= 2 ? 8 : 0;
 } else {
   p += vb === 0 ? 20 : vb <= 5 ? 15 : vb <= 20 ? 9 : 0;
 }
 p+=a.faults===0?15:a.faults<=2?10:a.faults<=6?5:0;
 if(a.unplugged>0)p=Math.min(p,25); if(ds>=7)p=Math.min(p,45);
 a.score=Math.max(0,Math.round(p));a.dias=ds;
 a.removalDates=uniqDates(a.removalDates);
 a.removalDatesText=a.removalDates.length?a.removalDates.slice(0,3).map(fmtDate).join(' | '):'-';
 a.lastRemovalDate=a.removalDates.length?fmtDate(a.removalDates[0]):'-';
 return a;
}
...
function topProblems(list){const s=summary(list);return[{label:'Sem dados 7d+',value:s.nodata,color:'#ef4444'},{label:'Tensão fora',value:s.voltage,color:'#f97316'},{label:'Falha GPS',value:s.gps,color:'#f59e0b'},{label:'Reboot excessivo',value:s.reboot,color:'#0ea5e9'},{label:'Possível remoção',value:s.unplugged,color:'#dc2626'}]}
